const apiURL = "https://api.openweathermap.org/data/2.5/weather?q=salt+lake+city&units=imperial&appid=803cd4031476c2dd2b8c10d59a9e1bde";
fetch(apiURL)
...
document.querySelector('#windspeed').textContent = `Windspeed: ${jsObject.wind.speed}`;
...
if(temp <= 50 && speed > 3){
    ...
    document.querySelector('#windchill').innerHTML = `Windchill: ${Math.round(windchill).toFixed(2)}`;
} else {
